...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        'password' => function($value) {
            return validateLength($value, 8, 50);
		},			
        'name' => function($value) {
      ...
      foreach ($new_user as $key => $value) {
		...
		if ($new_user['email'] == $user['email']) {
			$errors[$key] = "Данный e-mail уже используется";
		}
		
		$passwordHash = password_hash('password', PASSWORD_DEFAULT);
		if (!password_verify('password', $passwordHash)) {
		    $errors[$key] = "Пароль неверный";
		}
    }
	...
	if (count($errors)) {
		$page_content_sign_up = include_template('sign-up', ['errors' => $errors]);
	}
    else {
        $sqlNewUser = 'INSERT INTO users (id, registration_date, email, name, password, contacts) VALUES (?, NOW(), ?, ?, ?, ?)';
        $stmtNewUser = db_get_prepare_stmt($con, $sqlNewUser/*, $picture*/);
        $resNewUser = mysqli_stmt_execute($stmtNewUser);
...
$layout_content = include_template('layout.php', ['content' => $page_content, 'categories' => [], 'title' => 'Регистрация нового аккаунта']);
...
<div class="page-wrapper">
      ...
      <nav class="user-menu">
        <ul class="user-menu__list">
          <li class="user-menu__item">
            <a href="templates/sign-up.php">Регистрация</a>
          </li>
          <li class="user-menu__item">
  ...
  <main>
    ...
    <form class="form container form--invalid <?php if(count($errors)): ?>form--invalid<?php endif ?>" action="sign-up.php" method="post" autocomplete="off" enctype="multipart/form-data"> <!-- form
    --invalid -->
	  ...
	  <?php $classname = isset($errors['email']) ? "form__item--invalid" : ""; ?>
        ...
        <input class="<?= $classname; ?>" id="email" type="text" name="email" placeholder="Введите e-mail" value="<?= getPostVal('email'); ?>">
	  ...
	  <?php $classname = isset($errors['password']) ? "form__item--invalid" : ""; ?>
        ...
        <input class="<?= $classname; ?>" id="password" type="password" name="password" placeholder="Введите пароль" value="<?= getPostVal('password'); ?>">
	  ...
	  <?php $classname = isset($errors['name']) ? "form__item--invalid" : ""; ?>
        ...
        <input class="<?= $classname; ?>" id="name" type="text" name="name" placeholder="Введите имя" value="<?= getPostVal('name'); ?>">
	  ...
	  <?php $classname = isset($errors['name']) ? "form__item--invalid" : ""; ?>
        ...
        <textarea id="message" name="message" placeholder="Напишите как с вами связаться" class="<?= $classname; ?>"><?= getPostVal('contacts'); ?></textarea>
...
<footer class="main-footer">
  ...
  <div class="main-footer__bottom container">
    ...
    <a class="main-footer__add-lot button" href="templates/add.php">Добавить лот</a>
