...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    move_uploaded_file($_FILES['picture_img']['tmp_name'], 'uploads/' . $filename);

    $sql = 'INSERT INTO lots (id, date_create, name_lots, description, path, start_price, date_finish, bet_step, author_id, winner_id, category_name) VALUES (?, NOW(), ?, ?, ?, ?, ?, ?, 1, ?, ?)';
...
$sql_id = 'SELECT id, name_cat FROM categories';
...
<div class="page-wrapper">
  ...
  <main>
    ...
    <form class="form form--add-lot container form--invalid <?php if(count($errors)): ?>form--invalid<?php endif ?>" action="add.php" method="post" enctype="multipart/form-data"> <!-- form--invalid -->
      <h2>Добавление лота</h2>
      <div class="form__container-two">
        <div class="form__item form__item--invalid"> <!-- form__item--invalid -->
		
		 <?php $classname = isset($errors['name_lots']) ? "form__item--invalid" : ""; ?>
		 
          <label for="lot-name">Наименование <sup>*</sup></label>
		  <input class="<?= $classname; ?>" id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value="<?= getPostVal('name_lots'); ?>">
        ...
        <div class="form__item">
		  ...
		  <?php //foreach ($category as $value): ?>	<!-- мой вариант -->
          <select id="category" name="category" class="<?= $classname; ?>">
            <option>Выберите категорию</option>
			<?php foreach ($categories as $cat): ?>
            <option value="<?= $cat['id'] ?>" <?php if ($cat['id'] == getPostVal('category_id')): ?>selected<?php endif; ?>><?=$cat['name_cat']; ?><!--здесь было в тегах пхп $value['name_cat']; --><!--Доски и лыжи--></option>   <!-- закомментировано мой вариант -->
            <!--<option>Крепления</option>
            <option>Ботинки</option>
            <option>Одежда</option>
            <option>Инструменты</option>
            <option>Разное</option>-->
			<?php endforeach; ?>  <!-- из примера -->
          </select>
		  <?php //endforeach; ?> <!-- мой вариант -->
	  ...
	  <?php $classname = isset($errors['description']) ? "form__item--invalid" : ""; ?>
        ...
        <textarea id="message" name="message" placeholder="Напишите описание лота" class="<?= $classname; ?>"><?= getPostVal('description'); ?></textarea>
      ...
      <div class="form__container-three">
		...
		<?php $classname = isset($errors['start_price']) ? "form__item--invalid" : ""; ?>
          ...
          <input id="lot-rate" type="text" name="lot-rate" placeholder="0" class="<?= $classname; ?>" value="<?= getPostVal('start_price'); ?>">
		...
		<?php $classname = isset($errors['bet_step']) ? "form__item--invalid" : ""; ?>
          ...
          <input id="lot-step" type="text" name="lot-step" placeholder="0" class="<?= $classname; ?>" value="<?= getPostVal('bet_step'); ?>">
		...
		<?php $classname = isset($errors['date_finish']) ? "form__item--invalid" : ""; ?>
          ...
          <input class="form__input-date <?= $classname; ?>" id="lot-date" type="text" name="lot-date" placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?= getPostVal('date_finish'); ?>">
	  ...
	  <?php if (isset($errors)): ?>
        <div class="form__errors">
           <span class="form__error form__error--bottom">Пожалуйста, исправьте следующие ошибки:</span>   <!--  это я вставил из образца -->
           <ul>
     ...
     <?php endif; ?>
      ...
      <!--<span class="form__error form__error--bottom">Пожалуйста, исправьте ошибки в форме.</span>  это было изначально-->  
      <button type="submit" class="button" name="add">Добавить лот</button>
    </form>
  </main>
